...
export default function DirectoryClient({ coffeeFarms, teaFarms }: Props) {
  ...
  const handleTabChange = (next: FarmType) => {
    setTab(next); setSearch(''); setSelectedStates([]); setSelectedTags([]); setSelectedId(null)
  }

  const handleSelect = (id: string | null) => setSelectedId(prev => prev === id ? null : id)
  ...
  return (
    <div className="layout">
      <aside className="locations-sidebar">
        {selectedFarm ? (
          <>
            <button className="detail-back" onClick={() => setSelectedId(null)}>← Back</button>
